...
GitHub Actions run tests/builds and produce artifacts. Pushing to `main` automatically bumps the SemVer version, creates a git tag and GitHub Release, and publishes the package to PyPI via Trusted Publishing (see [PUBLISHING.md](docs/PUBLISHING.md)). The bump type is patch by default; append `[minor]` or `[major]` to the commit message to bump minor or major. Release commits start with `[skip ci]` so they do not re-trigger the release workflow. Configure protected environments and Trusted Publishing before the first push. Review permissions and current third-party dependency terms before distributing an APK. Debug signing on CI is ephemeral, so a CI APK may not update a locally signed installation without a signature conflict; use one controlled signing key for stable releases and never commit it. Do not uninstall an existing helper blindly to resolve a conflict if it contains data you need.
